...
while [ $i -le 59 ]; do
  ...
  blastn -query $IN/assembled/"$EXPT".assembled.fasta -db $(dirname $HOMOLOGSFILE)/blastdbs/$(basename $HOMOLOGSFILE) -max_hsps 1 -max_target_seqs -out $OUT/blastresults/"$EXPT".blastout.tsv -outfmt 6
